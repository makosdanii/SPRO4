...
float PID::calculatePID(float setpoint, float measured)
{
  ...
  // Calculate Filtered error
  float ef = alpha * error + (1 - alpha) * previous_ef;

  // Calculate Derivative term
  float derivative = (ef - previous_ef) / T_s;
  ...
  if(PIDoutput > max)
    ...
    integral += error * T_s; // Summing up the error over time (helps eliminate steady-state error) (only update integral if within saturation limits)

  // Save Filtered error to previous filtered error
  previous_ef = ef;
